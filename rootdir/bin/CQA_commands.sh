...
####################################################################
# Function:    SPEAKER_LEFT_ON                              #
# Description: This function should enable speaker left on   #
# Inputs:      N/A                                                 #
...
function SPEAKER_LEFT_ON
{
    ...
    adb shell am start -n com.ape.factory/.AuTestActivity -S -e speak 0
#    tinymix  Audio_PGA1_Setting 18Db
...
####################################################################
# Function:    SPEAKER_RIGHT_ON                              #
# Description: This function should enable speaker right on   #
# Inputs:      N/A                                                 #
# Output:      status:     PASS/FAIL                                #
####################################################################
function SPEAKER_RIGHT_ON
...
# Output:      OK,[cal data #comma delimited]/FAIL                #
####################################################################
function AUDIO_CAL_CQA
{
    # insert your code below
    am start -n com.motorola.motocit/.audio.SmarPACali
    sleep 4
    AUDIO_CAL="FAIL"
    for i in $(seq 15 -1 1)
    do
...
{
    # insert your code below
    echo "$(getprop persist.sys.SMART.PA.DATA)"
}
...
# Output:      status:     OK/FAIL                                #
####################################################################
function SENSOR_SAR_CHECK
{
    sleep 1
    SAR_CHECK="FAIL"
    RET=$(cat /sys/bus/i2c/drivers/aw9610x_sar/3-0012/diff)
	echo "$RET"
	FLAG="DIFF_CH4"
	if [[ $RET == *$FLAG* ]]
	then
...
function TOUCHPANEL_SELFTEST
{
	...
	PASS_WORD="Tp test pass"
	RET=$(cat /sys/class/leds/lcd-backlight/brightness)
	if [ $RET -eq 0 ];then
	input keyevent 26
	fi
	sleep 0.5
	RET=$(echo Conf_MultipleTest.ini > /sys/bus/spi/devices/spi2.0/fts_test)
    echo $RET
	sleep 2
...
# Description: This function should read the ro.boot.hardware.sku  #
# ro.boot.software.sku and ro.boot.product.hardware.sku            #
# Note:                                                            #
# Inputs:      N/A                                                 #
# Output:      status:     OK/FAIL                                 #
####################################################################
function READ_HW_SKU
{
    ...
    echo "[ro.boot.hardware.sku]:$(getprop ro.boot.hardware.sku)"
    echo "[ro.boot.software.sku]:$(getprop ro.boot.software.sku)"
    echo "[ro.boot.product.hardware.sku]:$(getprop ro.boot.product.hardware.sku)"
}
##### Main #####
eval $@
